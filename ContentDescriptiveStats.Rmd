...
//WORKING ON THIS!! loop through final sample df and use API to save ArticleContent
ArticleContent<- vector(mode='character', length=dim(LFTaste300)[1])
ArticleTitle<- vector(mode='character', length=dim(LFTaste300)[1])
for (i in 1:dim(LFTaste300[1]) { h<-GET(paste0("http://apps.asc.upenn.edu/tcors2/api/articles?article_list=",     LFTaste300$ArticleID[i])) h1=content(h)
  
  ArticleContent[i]<- as.character(h1[[1]][3])
  ArticleTitle[i]<- as.character(h1[[1]][2])
}
